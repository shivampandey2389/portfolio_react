const Skills = () =>{
  ...
  <div className="flex flex-col items-start justify-start w-[80%] my-[20px]">
    <h1 className="text-white text-[45px] tracking-wider">Skills</h1>
      ...
      <div className="flex flex-wrap justify-center gap-2.5 w-[min(100%,800px)]">
        {
          imageTitle.map((item)=>{
            return(
              ...
              <img src={item.url} alt={item.name} className={item.name =="React"?"h-15 spinIn":"h-15"} />
              <span className="text-3xl text-[#E0E0E0] ">{item.name}</span>
            </div>
            )
          })
        }
